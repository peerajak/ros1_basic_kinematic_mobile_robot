...
class PID_controller():
    ...
    def plan_process(self, controller_signal):
        global odometry
...
def simulate_pid(duration, pid_controller, xg, yg, thetag ,threshold, threshold_angle):
    global odometry
    dt = 0.1    # create a list of times from 0 to `duration` seconds with steps of `self.dt`
    # in total, duration / self.dt observations will be recorded
    ...
    while(distance_error_norm > threshold or error_angle > threshold_angle):
        ...
        output_signal = pid_controller.plan_process(res)
